...
SEASON_UNITS.push(
  mk({id:'ryujin',name:'용왕',short:'용왕',role:'조수 지배',shape:'ryujin',castFx:'tidewave',season:'deepcourt',rarity:'UR',gacha:true,unlockStage:999,
    ...
    ab:{slow:2.0,push:20,weaken:{mul:.86,dur:2.5}},
    abText:'범위 · 2초 둔화 · 밀쳐 냄 · 맞은 적 피해 14% 감소',
...
function enemyTactic(e) {
  ...
  // 뭘 하는 놈인지가 먼저다. 갑주는 그 다음 문제다.
  if (ab.summon) return '증원 소환 · 불어나기 전에 본체를 끊어라';
  if (ab.rally) return '주변 적 강화 · 후열부터 도려내라';
  if (ab.weaken) return '아군 피해 감소 · 사거리 긴 병종으로 먼저 끊어라';
  if (ab.backline) return '뒷줄 급습 · 원거리 병종을 가려 줄 앞줄이 필요';
  if (ab.hold) return '고정 포대 · 전진하지 않는다. 몰아쳐서 부숴라';
  if (ab.thorns) return '근접 반격 · 원거리 공격으로 대응';
